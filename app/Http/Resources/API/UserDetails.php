...
class UserDetails extends JsonResource
{
    ...
    {
        $attributes = $this->attributes->groupBy('attribute_key');
        return [
            'id'=>$this->id,
            'name'=>$this->name??"",
            'email'=>$this->email,
            ...
            'profile_video'=>asset('public/uploads/'.$this->profile_video),
            ...
            'like_request_status'=>auth()->check??Auth::user()->getFriendShipStatus(User::find($this->id))
